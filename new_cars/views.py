...
df = pd.read_csv("/Users/sajagagrawal/Documents/Projects/Bharatrides_backend/Bhrides_back/new_cars/indian_car_database_1m_full.csv")
...
def recommend_cars_api(request):
    ...
    try:
        ...
        min_price_lakhs = data["min_price"] / 100000
        max_price_lakhs = data["max_price"] / 100000
        print(filtered)
        if filtered.empty:
        ...
        filtered = filtered[
         (filtered["predicted_price"] >= min_price_lakhs) &
         (filtered["predicted_price"] <= max_price_lakhs)
        ].copy()
        ...
        filtered["price_diff"] = abs(filtered["predicted_price"] - filtered["price (in lakhs)"])
    ...
    "seating_capacity", "price (in lakhs)", "predicted_price"
